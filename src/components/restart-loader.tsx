...
export function RestartLoader({ onComplete }: { onComplete?: () => void }) {
  ...
  const filledPages = Math.max(0, Math.min(totalPages, Math.floor(progress * totalPages)));
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-background/95 backdrop-blur-md">
      <div className="relative h-40 w-56" aria-hidden>
        <div className="absolute inset-0 rounded-r-lg bg-primary shadow-xl" />
        ...
        {Array.from({ length: totalPages }).map((_, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(
                "absolute right-1 top-1 h-[calc(100%-8px)] w-[calc(100%-12px)] origin-left rounded-r border border-border bg-card shadow-sm transition-all duration-500 ease-out",
                isActive ? "opacity-100 translate-x-0 rotate-0" : "opacity-0 translate-x-4 -rotate-6"
              )}
        ...
        <div className="absolute -bottom-6 left-1/2 h-1 w-44 -translate-x-1/2 overflow-hidden rounded-full bg-muted">
          <div
