...
export function FavoriteButton({ id, className, size = "md" }: { id: string; className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <button
      ...
      aria-pressed={fav}
      className={cn(
        "relative inline-flex items-center justify-center rounded-full border bg-background/90 backdrop-blur transition-all hover:scale-110 shadow-sm",
        dims,
        fav ? "border-rose-300 text-rose-500" : "border-border text-muted-foreground hover:text-rose-500 hover:border-rose-300",
        className,
      )}
    >
      <Heart key={burst} className={cn(icon, fav && "fill-rose-500", burst > 0 && "animate-heart-pop")} />
      {burst > 0 && fav && (
        <span
          key={`burst-${burst}`}
          aria-hidden
          className="pointer-events-none absolute inset-0 rounded-full border-2 border-rose-400/70 animate-heart-burst"
        />
      )}
    </button>
  );
