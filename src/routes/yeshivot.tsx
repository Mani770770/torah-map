          ...
          <main className="flex-1 min-w-0">
            ...
            {filtered.length === 0 ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {filtered.map(y => (
                  <article key={y.id} className="rounded-xl border border-border bg-card p-5 transition-shadow hover:shadow-md">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="text-lg font-bold text-foreground">{y.name}</h3>
                    ...
                    <p className="mt-3 text-sm text-muted-foreground line-clamp-3">{y.description}</p>
                  </article>
                ))}
              </div>
            )}
